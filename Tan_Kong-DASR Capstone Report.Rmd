---
title: "2020 Singapore General Election Sentiments across Two Social Media Platforms"
author: "David Tan & Fabian Kong"
date: "02 August 2020"
output:
  html_document:
    theme: flatly
...
![Singapore Parlimentary General Election Process and its Phases](Tan_Kong-election.png)
...
Chatter plots were used to display the most frequently occurring words against their emotional valence [^2]. 
...
Both have the most word used in the posts - "winning". The context of this most used word is likely on the hope in winning the constituency the candidates that are contesting in. Moving later into the election process, the winning candidates would be happy, and the democracy in Singapore has made progress with the official appointment of the Leader of the Opposition [^3]. Both words "happy" and "progress" are the other top few positive words in Twitter.

In contrast, the top few most negative words in Twitter are "bad", "lost" and "disappointed". These words are likely due to candidates that are hopeful to win but did not in the end. This may also be related to another GRC lost by the incumbent [^4].
...
Based on the AFINN sentiment plots, the trend for the opposition is generally similar between Reddit and Twitter with a dip to negative between 4 and 5 July. This is probably due to the police investigation on Workers' Party candidate Raeesah Khan over alleged online comments on race and religion [^5]. There is also a dip to negative for the incumbent party in Twitter, due to the withdrawal of PAP candidate Ivan Lim after allegations about his past conduct and behaviour [^6]. 
...
ANOVA models remain robust even if the assumption of homogeneity of variance is violated. Robustness is dependent on the sample sizes across groups [^7]. In our case, the sample sizes across groups are not similar to one another. Hence, we have to use a non-parametric equivalent to the 3-way between-groups ANOVA.
...
The `t3way` function from the `WRS2` package was used as a robust 3-Way Between-Groups ANOVA alternative [^8]. The results showed that there was a significant three-way interaction between `platform`, `period_2020`, and `pol_party`, p = 0.0010.
...
A significant three-way interaction could be followed up with the following post-hoc tests [^9]:
...
[^1]: [Election Department Singapore](www.eld.gov.sg/)
[^2]: [RIP wordclouds, long live CHATTERPLOTS](https://towardsdatascience.com/rip-wordclouds-long-live-chatterplots-e76a76896098)
[^3]: [GE2020: WP chief Pritam Singh says he looks forward to serving as Leader of the Opposition](https://www.channelnewsasia.com/news/singapore/ge2020-wp-chief-pritam-singh-says-he-looks-forward-to-serving-as-12924456)
[^4]: [PAP lost the youth vote in Sengkang GRC](https://mothership.sg/2020/07/pap-lost-sengkang-grc-why/)
[^5]: [Police investigating WP candidate Raeesah Khan over alleged online comments on race, religion](https://www.channelnewsasia.com/news/singapore/raeesah-khan-workers-party-police-reports-race-ge2020-sengkang-12903248)
[^6]: [Singapore GE 2020: PAP new face Ivan Lim withdraws from election following allegations about his past behaviour](https://www.straitstimes.com/singapore/singapore-ge-2020-pap-new-face-ivan-lim-withdraws-as-a-candidate)
[^7]: [The Problem with Using Tests for Statistical Assumptions](https://www.theanalysisfactor.com/the-problem-with-tests-for-statistical-assumptions/)
[^8]: [WRS2: A Collection of Robust Statistical Methods](https://cran.r-project.org/web/packages/WRS2/index.html)
[^9]: [ANOVA in R](https://www.datanovia.com/en/lessons/anova-in-r/)
[^10]: [FAQ How Can I Understand A Three-way Interaction In ANOVA?](https://stats.idre.ucla.edu/other/mult-pkg/faq/general/faqhow-can-i-understand-a-three-way-interaction-in-anova/)